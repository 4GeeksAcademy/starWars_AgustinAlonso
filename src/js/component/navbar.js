...
export const Navbar = () => {
    ...
    store.favorites.map((fav, index) => (
      <li key={index} className='d-flex justify-content-between dropdown-item align-items-center'><div>{fav.name}</div><div ><ion-icon style={{ fontSize: '24px' }} name="heart-dislike"></ion-icon></div></li>
    ))
  )}
						</ul>
					</div>
